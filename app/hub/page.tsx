    ...
    <main className="mesh-bg min-h-screen flex flex-col">
      ...
      <header style={{ padding: '16px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid rgba(124,58,237,0.1)', position: 'sticky', top: 0, zIndex: 40, background: 'rgba(255,255,255,0.85)', backdropFilter: 'blur(20px)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <span style={{ fontSize: '18px', fontWeight: '700', letterSpacing: '-0.02em', color: '#111111' }}>vonage</span>
          {contactName && (
            <span style={{ marginLeft: '8px', fontSize: '13px', color: 'rgba(0,0,0,0.4)' }}>
              · {contactName}
            </span>
          )}
        </div>
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '40px 24px', width: '100%' }}>
        ...
        <div className="animate-fade-up" style={{ marginBottom: '40px' }}>
          ...
          <h1 style={{ fontSize: 'clamp(26px, 5vw, 42px)', fontWeight: '700', letterSpacing: '-0.02em', marginBottom: '12px', color: '#111111' }}>
            {t(lang, 'hubTitle')}
          </h1>
          <p style={{ color: 'rgba(0,0,0,0.5)', fontSize: '16px', maxWidth: '520px', lineHeight: '1.6' }}>
            {t(lang, 'hubSubtitle')}
          </p>
        </div>
        ...
        <div className="glass-card animate-fade-up" style={{ padding: '32px', marginBottom: '32px', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: '16px' }}>
          {meetingRequested ? (
            <>
              ...
              <p style={{ color: 'rgba(0,0,0,0.55)', fontSize: '15px' }}>{t(lang, 'meetingConfirm')}</p>
            ...
            <>
              ...
              <p style={{ color: 'rgba(0,0,0,0.55)', fontSize: '15px', maxWidth: '400px' }}>{t(lang, 'meetingSubtitle')}</p>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '16px', padding: '20px', opacity: 0.4, flexWrap: 'wrap' }}>
          ...
          <span style={{ fontSize: '12px', color: 'rgba(0,0,0,0.4)' }}>CX Tour KL 2026 · 23 June · W Kuala Lumpur</span>
        </div>
      </div>
      ...
      {chatOpen && (
        <div style={{
          ...
          <div style={{ padding: '16px 20px', borderBottom: '1px solid rgba(124,58,237,0.1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(124,58,237,0.06)' }}>
            <div>
              <div style={{ fontWeight: '600', fontSize: '14px', color: '#111111' }}>{t(lang, 'chatTitle')}</div>
            ...
            <button onClick={() => setChatOpen(false)} style={{ background: 'none', border: 'none', color: 'rgba(0,0,0,0.5)', cursor: 'pointer', fontSize: '20px', lineHeight: 1 }}>×</button>
          </div>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            {chatLoading && (
              <div className="chat-bubble-ai" style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
                {[0,1,2].map(i => (
                  <div key={i} style={{ width: '6px', height: '6px', borderRadius: '50%', background: 'rgba(0,0,0,0.3)', animation: `dot-bounce 1.2s ease-in-out ${i*0.2}s infinite` }} />
                ))}
              </div>
            )}
            <div ref={chatEndRef} />
  ...
  return (
    <div className="product-card animate-fade-up" style={{ padding: '28px', cursor: 'pointer' }} onClick={onToggle}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '16px' }}>
        ...
        <span style={{ fontSize: '18px', color: 'rgba(0,0,0,0.3)', transform: expanded ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s', display: 'block', marginTop: '4px' }}>⌄</span>
      </div>

      <p style={{ fontSize: '14px', color: 'rgba(0,0,0,0.55)', lineHeight: '1.6', marginBottom: expanded ? '20px' : '0' }}>{desc}</p>
      ...
      {expanded && (
        <div style={{ marginTop: '20px', animation: 'fadeIn 0.2s ease' }} onClick={e => e.stopPropagation()}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '20px' }}>
            {benefits.map((b, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
                ...
                <span style={{ fontSize: '13px', color: 'rgba(0,0,0,0.65)', lineHeight: '1.5' }}>{b}</span>
              </div>
            ))}
          </div>
          ...
          <div style={{ borderTop: '1px solid rgba(124,58,237,0.1)', paddingTop: '16px' }}>
            <div style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.08em', color: 'rgba(0,0,0,0.3)', marginBottom: '10px', fontWeight: '600' }}>
              {resourcesLabel}
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
              {resources.map((r, i) => (
                <a
                  ...
                  style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '7px 14px', borderRadius: '8px', background: `${accentColor}15`, border: `1px solid ${accentColor}30`, color: 'rgba(0,0,0,0.7)', fontSize: '13px', textDecoration: 'none', transition: 'all 0.15s' }}
